...
PaperFigureDirec = ['/Users/' UserName '/Dropbox/HDMP2RAGE_figures/SupplemetaryMaterial']


PDFFileName = fullfile(PaperFigureDirec,'PropertiesFor7TAnd512Slices.pdf');
...
SlicesPerSlab               = 512;
MP2RAGE(1).B0               = 7;                    % B0 in Tesla
MP2RAGE(1).TR               = 6;                    % MP2RAGE(d) TR in seconds
MP2RAGE(1).TRFLASH          = 3.9E-3;              % TEcho spacing in ms
...
PartialFourierInSlice       = .75;                  % Partial Fouirer in slice
SlicesPerSlab               = 512;
MP2RAGE(2).B0               = 7;                    % B0 in Tesla
MP2RAGE(2).TR               = 6;                    % MP2RAGE(d) TR in seconds
MP2RAGE(2).TRFLASH          = 3.9E-3;              % TEcho spacing in ms
MP2RAGE(2).TIs              = [500E-3 2000E-3];     % TI for INV1 and INV2 % INV1 vas should be 500 this is changed in first revision
...
elseif MP2RAGE(1).B0==7 % examples of T1 values at 7T
    T1WM    = 1.1;
    T1GM    = 1.85;
    T1CSF   = 3.9;
    T1YTickVec = [0 0.5 T1WM 1.7 T1GM 2 T1CSF 5];
end
...
title(MP2RAGE(1).name,'FontWeight','bold')
ylim([0 5])
StandardAx = gca;
...
xlabel('UNI')
text(-0.22,1.3,['B_1^+: ' num2str(round(100*B1range(1))) '%'],'Rotation',-25,'FontWeight','bold')
text(-0.22,2.1,['B_1^+: ' num2str(round(100*B1range(end))) '%'],'Rotation',-25,'FontWeight','bold')
...
title(MP2RAGE(2).name,'FontWeight','bold')
ylim([0 5])
LessBiasAx = gca;
...
text(-0.42,1.3,['B_1^+: ' num2str(round(100*B1range(1))) '%'],'Rotation',-25,'FontWeight','bold')
text(-0.42,2.1,['B_1^+: ' num2str(round(100*B1range(end))) '%'],'Rotation',-25,'FontWeight','bold')
...
%%
axes(NewStandardAx)
atext = text(-0.5,5.25,'a','FontSize',40,'FontName','Times','FontWeight','bold')

axes(NewLessBiasAx)
atext = text(-0.5,5.25,'b','FontSize',40,'FontName','Times','FontWeight','bold')
