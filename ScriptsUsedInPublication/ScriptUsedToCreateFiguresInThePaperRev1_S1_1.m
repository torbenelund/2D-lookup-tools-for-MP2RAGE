...
CopyFiles               = 0; %set to 1 if converted files needs to be copied
...
m                       = spm_matrix([-0.6891  -15.6450  -13.6256],'R*T');
...
if DoRecon
    ...
    D = [{'Standard_FA060'} {'Standard_FA100'} {'Standard_FA140'} {'LessBias_FA060'} {'LessBias_FA100'} {'LessBias_FA140'}];
    P = [...
        'meas_MID00056_FID05235_PRJ0032_MP2RAGE_ES7_18_1_Standard_FA060.dat    '
        'meas_MID00054_FID05233_PRJ0032_MP2RAGE_ES7_18_1_Standard_FA100.dat    '
        'meas_MID00058_FID05237_PRJ0032_MP2RAGE_ES7_18_1_Standard_FA140.dat    '
        'meas_MID00057_FID05236_PRJ0032_MP2RAGE_ES7_18_9_LessGMWMBias_FA060.dat'
        'meas_MID00055_FID05234_PRJ0032_MP2RAGE_ES7_18_9_LessGMWMBias_FA100.dat'
        'meas_MID00059_FID05238_PRJ0032_MP2RAGE_ES7_18_9_LessGMWMBias_FA140.dat']
...
subdirs     = spm_select('List',MainDirec,'dir','FA_*.');
FPsubdirs   = spm_select('FPList',MainDirec,'dir','FA_*.');
...
if DoCoregister
    RefIdx      = contains({MP2RAGE.filenamecalcR1_1DLUT},'Standard_FA100');
    clear matlabbatch
    
    for d = 1 : size(subdirs,1)
        
        if ~strcmp(subdirs(d,:),'Standard_FA100')
...
if CAT12Segment
   % pool=parpool(6,"IdleTimeout",120)
    P = char({...
        fullfile(R1Direc,'^mcalcR1_1D-LUTStandard_FA060.nii')...
        fullfile(R1Direc,'^mcalcR1_1D-LUTStandard_FA100.nii')...
        fullfile(R1Direc,'^mcalcR1_1D-LUTStandard_FA140.nii')...
        fullfile(R1Direc,'^mcalcR1_2D-LUTLessBias_FA060.nii')...
        fullfile(R1Direc,'^mcalcR1_2D-LUTLessBias_FA100.nii')...
        fullfile(R1Direc,'^mcalcR1_2D-LUTLessBias_FA140.nii')});
    %par
    for i = 1:size(P,1)
...
if MakeFig1
    ...
    StandardFA100Idx      = contains({MP2RAGE.filenameUNI},'Standard_FA100');
    LessBiasFA100Idx      = contains({MP2RAGE.filenameUNI},'LessBias_FA100');
...
if MakeFig2
    MapIdx      = contains({MP2RAGE.filenameUNI},'LessBias_FA100');
    ...
    %P = char({MP2RAGE(MapIdx).filenameScaledUNI MP2RAGE(MapIdx).filenameDSR MP2RAGE(MapIdx).filenamecalcR1_1DLUT MP2RAGE(MapIdx).filenamecalcR1_2DLUT})
     ...
     P = char({...
        fullfile(R1Direc,'ScaledUNI_LessBias_FA100_UNI.nii')...
        fullfile(R1Direc,'DSR_LessBias_FA100_UNI.nii')...
        fullfile(R1Direc,'bmcalcR1_1D-LUTStandard_FA100.nii')...
        fullfile(R1Direc,'bmcalcR1_2D-LUTLessBias_FA100.nii')...
        });
...
if MakeFig3
    spm_check_registration(char({...
        fullfile(R1Direc,'bmcalcR1_1D-LUTStandard_FA060.nii')...
        fullfile(R1Direc,'bmcalcR1_2D-LUTLessBias_FA060.nii')...
        fullfile(R1Direc,'bmcalcR1_1D-LUTStandard_FA100.nii')...
        fullfile(R1Direc,'bmcalcR1_2D-LUTLessBias_FA100.nii')...
        fullfile(R1Direc,'bmcalcR1_1D-LUTStandard_FA140.nii')...
        fullfile(R1Direc,'bmcalcR1_2D-LUTLessBias_FA140.nii')...
...
% for i=1:2:6,Y=spm_read_vols(spm_vol(Pcorr(i,:)));subplot(2,1,1),histogram(Y(Y(:)>0),'BinLimits',[0 2],'NumBins',100,'DisplayStyle','Stairs','LineWidth',2),hold on,grid on,ylim([0 1E5]);set(gca,'XTick',[0:0.1:2]);end
% figure,for i=2:2:6,Y=spm_read_vols(spm_vol(P(i,:)));subplot(2,1,2),histogram(Y(Y(:)>0),'BinLimits',[0 2],'NumBins',100,'DisplayStyle','Stairs','LineWidth',2),hold on,grid on,ylim([0 1E5]);set(gca,'XTick',[0:0.1:2]);end
% for i=1:2:6,Y=spm_read_vols(spm_vol(P(i,:)));subplot(2,1,1),histogram(Y(Y(:)>0),'BinLimits',[0 2],'NumBins',100,'DisplayStyle','Stairs','LineWidth',2),hold on,grid on,ylim([0 1E5]);set(gca,'XTick',[0:0.1:2]);end
% P