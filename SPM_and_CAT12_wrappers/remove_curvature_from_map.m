...
R1MeshName          = deblank(R1MeshName);
CurvatureMeshName   = deblank(CurvatureMeshName);
...
%tst = double(r1.vertices);
%idx=find(isfinite(C).*isfinite(R1) .* (tst(:,1)<=0));

%find indices for finite values (both Hemispheres)
