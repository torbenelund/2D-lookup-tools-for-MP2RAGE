...
MainDirec               = '/Volumes/LaCie#1/Data/HDRMP2RAGE/B1BiasHuman/NIFTI';
RawDirec                = '/Volumes/LaCie#1/Data/HDRMP2RAGE/B1BiasHuman/MP2RAGE_FatNavs';
R1Direc                 = '/Volumes/LaCie#1/Data/HDRMP2RAGE/B1BiasHuman/NIFTI/R1maps_step0_001_coreg';
SurfDirec               = [R1Direc filesep 'surf'];
mkdir(R1Direc)

DoRecon                 = 1; %set to 1 for conversion of rawdata
...
m                       = spm_matrix([-1.6 -15.9 -13.6 0.08],'R*T');
...
if DoRecon
    ...
    for i=1:size(P,1)
        ...
        reconstructSiemensMP2RAGEwithFatNavs(rawDataFile,'FatNavRes_mm',4,'bZipNIFTIs',1,'bKeepReconInRAM',1,'bGRAPPAinRAM',1,'outRoot',outroot);
    end
end
...
for d=1:size(subdirs,1)
    ...
    elseif contains(subdirs(d,:),'LessBias')
        ...
        MP2RAGE(d).TR               = 5;                    % MP2RAGE(d) TR in seconds
        MP2RAGE(d).TRFLASH          = 7.18E-3;              % TEcho spacing in ms
        MP2RAGE(d).TIs              = [550E-3 1900E-3];     % TI for INV1 and INV2
        MP2RAGE(d).NZslices         = SlicesPerSlab * [PartialFourierInSlice-0.5  0.5];
        ...
        if ~strcmp(subdirs(d,:),'Standard_FA100')
            ...
            if contains(MP2RAGE(d).name,'LessBias')
                ...
                OtherImage = char({MP2RAGE(d).filenameINV1 MP2RAGE(d).filenameINV2 MP2RAGE(d).filenameUNI ...
                    ...
                    MP2RAGE(d).filenamecalcT1_1DLUT MP2RAGE(d).filenamecalcR1_1DLUT spm_file(SourceImage,'prefix','m')});
